class ApplicationController < GenericApplicationController
      ...
      case type
        ...
        when 'DISPENSING'
          encounter_hiv_clinic_consultation = Encounter.find(:first,:conditions =>["patient_id = ? AND encounter_type = ? AND DATE(encounter_datetime) = ?",
                                   patient.id,EncounterType.find_by_name('HIV CLINIC CONSULTATION').id,session_date],
                                   :order =>'encounter_datetime DESC,date_created DESC',:limit => 1)
          next unless encounter_hiv_clinic_consultation.observations.map{|obs| obs.to_s.strip.upcase }.include? 'Prescribe drugs:  Yes'.upcase rescue true
  ...
  def main_next_task(location, patient, session_date = Date.today)
    ...
    all_tasks.each do |task|
      ...
      if task.encounter_type == 'HIV CLINIC CONSULTATION' and (PatientService.reason_for_art_eligibility(patient).blank? or PatientService.reason_for_art_eligibility(patient).match(/unknown/i))
        skip = true
      end
      
      if task.encounter_type == 'HIV STAGING' and not (PatientService.reason_for_art_eligibility(patient).blank? or PatientService.reason_for_art_eligibility(patient).match(/unknown/i))
        skip = true
      end
