...
=end
  ...
  def self.occupations
	...
	## Block added to enable processing of FCIG encoded guidelines for alerts and reminders
	# Pulling concept sets
	#TODO move concept set method from medication service
=begin
...
=end
	# Evaluate condition
	def self.evaluate_concept_condition(variable, relator, variable_value, patient, unit=nil)
		valid = true		

        if (variable_value).casecmp('true') == 0
            variable_value = 'Yes'
        end

        if (variable_value).casecmp('present') == 0
            variable_value = 'Yes'
        end

        if (variable_value).casecmp('false') == 0
            variable_value = 'No'
        end

		if is_number?(variable_value) #.is_a? Numeric)
			if (relator).casecmp('is') == 0 || (relator).casecmp('=') == 0
				if !(patient_data(variable, patient, unit).gsub('<','').gsub('<=','').gsub('>','').gsub('>=','').gsub('=','').to_f == variable_value.to_f)
					return false
				end
			end
			
			if (relator).casecmp('>') == 0
				if !(patient_data(variable, patient, unit).gsub('<','').gsub('<=','').gsub('>','').gsub('>=','').gsub('=','').to_f > variable_value.to_f)
					return false
				end
			end
			...
			if (relator).casecmp('is') == 0 || (relator).casecmp('=') == 0
				if patient_data(variable, patient, unit).strip.casecmp(variable_value.strip) != 0

					variable_value_set = CoreService.concept_set(variable_value) #DeterminingArtEligibility.concept_set(variable_value)

					matching_condition = false
					if !variable_value_set.nil?
						variable_value_set.each do | v_value |
							if patient_data(variable, patient, unit).strip.casecmp(v_value[0].strip) == 0
			...
			if (relator).casecmp('>') == 0
				if !(patient_data(variable, patient, unit) > variable_value)
					return false
				end
			end
	...
	# Return original name if concept not found
	def self.concept_value(concept_name)
		concept = ConceptName.find_by_name(concept_name).concept_id rescue nil

		if !concept.nil?
		    answer = Concept.find_by_concept_id(concept).shortname rescue nil
	
			if answer.nil?
				answer = Concept.find_by_concept_id(concept).fullname rescue nil
			end

			if answer.nil?
				answer = Concept.find_with_voided(concept).fullname + ' - retired'
			end			
			return answer
		end
	...
	# Pull out the most recent observation, patient attribute or program status for the concept
	def self.patient_data(concept_name, patient, unit=nil)
		
		if concept_name.casecmp('age') == 0
			if unit.casecmp('Years') == 0
				return patient.age.to_s
			else
				if unit.casecmp('Months') == 0
					return patient.age_in_months.to_s
				else
					return (patient.age_in_months * 4).to_s
				end
			end
		end

		if concept_name.casecmp('HIV rapid test result') == 0
			return PatientService.patient_hiv_status(Patient.find_by_patient_id(patient.patient_id))
		end

		if concept_name.casecmp('On ART') == 0
			program_id = Program.find_by_name("HIV PROGRAM").id
    		location_id = Location.current_health_center.location_id
			last_patient_hiv_program = PatientProgram.find(:all,:conditions =>["voided = 0 AND patient_id = ? AND program_id = ? AND location_id = ?", patient.patient_id , program_id, location_id]).last rescue nil
			on_art = 'No'
			if !last_patient_hiv_program.nil?
				if ProgramWorkflowState.find_state(last_patient_hiv_program.patient_states.last.state).concept.fullname == "On antiretrovirals"
					on_art = 'Yes'
				end
			end
			return on_art			
		...
		current_patient = Patient.find(patient.patient_id) rescue nil
		current_encounters = current_patient.encounters
    	concept_id = Concept.find_by_name(concept_name).concept_id rescue nil

     	recent_answer = Observation.find(:first,
      							:conditions => ["obs.concept_id = ? AND obs.voided = 0 AND obs.person_id = ?", concept_id, patient.patient_id],
								:order => "obs_datetime DESC").answer_string rescue nil
